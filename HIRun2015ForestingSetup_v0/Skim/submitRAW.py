...
for line in f:
    if not line.find('root://') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cd -\n')
            scriptFile.write('cp %s/unpackT0Streamer.py %s \n' % (workBase,jobsBase))
            scriptFile.write('cp %s/unpackT0Streamer.py .\n' % workBase)
            # scriptFile.write('cp $CMSSW_BASE/src/HeavyIonsAnalysis/JetAnalysis/test/dbFiles/*.db .\n')
            scriptFile.write('cmsRun unpackT0Streamer.py outputFile=RAW_%d.root maxEvents=100 inputFiles=%s\n' % (jobCounter,line) )
            scriptFile.write('cmsMkdir %s\n' % outdir)
            scriptFile.write('ls\n')
            scriptFile.write('cmsStage -f RAW_%d_numEvent100.root %s/RAW_%d.root\n' % (jobCounter,outdir,jobCounter) )
            scriptFile.write('rm RAW_%d*root\n' % (jobCounter))
            scriptFile.close()
