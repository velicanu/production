...
for line in f:
    if not line.find('root://') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cd -\n')
            scriptFile.write('cp %s/RunExpressProcessingCfg.py %s \n' % (workBase,jobsBase))
            scriptFile.write('cp %s/RunExpressProcessingCfg.py .\n' % workBase)
            # scriptFile.write('cp $CMSSW_BASE/src/HeavyIonsAnalysis/JetAnalysis/test/dbFiles/*.db .\n')
            scriptFile.write('cmsRun RunExpressProcessingCfg.py outputFile=recoExpress_%d.root maxEvents=-1 inputFiles=%s\n' % (jobCounter,line) )
            scriptFile.write('cmsMkdir %s\n' % outdir)
            scriptFile.write('ls\n')
            scriptFile.write('cmsStage -f recoExpress_%d.root %s/recoExpress_%d.root\n' % (jobCounter,outdir,jobCounter) )
            scriptFile.write('rm recoExpress_%d*root\n' % (jobCounter))
            scriptFile.close()
