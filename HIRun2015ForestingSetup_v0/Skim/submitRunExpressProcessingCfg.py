...
for line in f:
    if not line.find('root://') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cd -\n')
            scriptFile.write('cp %s/RunExpressProcessingCfgZDCFix.py %s \n' % (workBase,jobsBase))
            scriptFile.write('cp %s/RunExpressProcessingCfgZDCFix.py .\n' % workBase)
            # scriptFile.write('cp $CMSSW_BASE/src/HeavyIonsAnalysis/JetAnalysis/test/dbFiles/*.db .\n')
            scriptFile.write('cmsRun RunExpressProcessingCfgZDCFix.py outputFile=FEVT_%d.root maxEvents=201 inputFiles=%s\n' % (jobCounter,line) )
            scriptFile.write('cmsMkdir %s\n' % outdir)
            scriptFile.write('ls\n')
            scriptFile.write('cmsStage -f FEVT_%d_numEvent201.root %s/FEVT_%d_numEvent201.root\n' % (jobCounter,outdir,jobCounter) )
