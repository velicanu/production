...
for line in f:
    if not line.find('root://') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cmsRun RunExpressProcessingCfgonRAW.py outputFile=FEVT_%d.root maxEvents=1501 inputFiles=%s\n' % (jobCounter,line) )
            scriptFile.write('cmsMkdir %s\n' % outdir)
            scriptFile.write('ls\n')
            scriptFile.write('cmsStage -f FEVT_%d_numEvent1501.root %s/FEVT_%d_numEvent1501.root\n' % (jobCounter,outdir,jobCounter) )
            scriptFile.write('rm FEVT_%d_numEvent1501.root\n' % (jobCounter))
            scriptFile.close()
