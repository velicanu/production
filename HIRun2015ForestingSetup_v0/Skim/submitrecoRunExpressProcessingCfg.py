...
for line in f:
    if not line.find('root://') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cd -\n')
            scriptFile.write('cp %s/recoRunExpressProcessingCfg.py %s \n' % (workBase,jobsBase))
            scriptFile.write('cp %s/recoRunExpressProcessingCfg.py .\n' % workBase)
            # scriptFile.write('cp $CMSSW_BASE/src/HeavyIonsAnalysis/JetAnalysis/test/dbFiles/*.db .\n')
            scriptFile.write('cmsRun recoRunExpressProcessingCfg.py outputFile=RECO_%d.root maxEvents=1001 inputFiles=%s\n' % (jobCounter,line) )
            scriptFile.write('cmsMkdir %s\n' % outdir)
            scriptFile.write('ls\n')
            scriptFile.write('cmsStage -f RECO_%d_numEvent1001.root %s/RECO_%d_numEvent1001.root\n' % (jobCounter,outdir,jobCounter) )
            scriptFile.write('rm RECO_%d_numEvent1001.root\n' % (jobCounter))
            scriptFile.close()
