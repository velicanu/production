...
for line in f:
    if not line.find('root://') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cd -\n')
            scriptFile.write('cp %s/runUnpackerEmulatorBitAnalyzer.py %s \n' % (workBase,jobsBase))
            scriptFile.write('cp %s/runUnpackerEmulatorBitAnalyzer.py .\n' % workBase)
            # scriptFile.write('cp $CMSSW_BASE/src/HeavyIonsAnalysis/JetAnalysis/test/dbFiles/*.db .\n')
            scriptFile.write('cmsRun runUnpackerEmulatorBitAnalyzer.py outputFile=HiForest_%d.root maxEvents=-1 inputFiles=%s\n' % (jobCounter,line) )
