...
for line in f:
    if not line.find('root://eoscms//eos/cms') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cd -\n')
            scriptFile.write('cp %s/runForest_PbPb_DATA_75X_Express.py %s \n' % (workBase,jobsBase))
            scriptFile.write('cp %s/hireco_RAW2DIGI_L1Reco_RECO.py %s \n' % (workBase,jobsBase))
            scriptFile.write('cp %s/runForest_PbPb_DATA_75X_Express.py .\n' % workBase)
            scriptFile.write('cp %s/hireco_RAW2DIGI_L1Reco_RECO.py .\n' % workBase)
#            scriptFile.write('cp $CMSSW_BASE/src/HeavyIonsAnalysis/JetAnalysis/test/*.db .\n')
            scriptFile.write('cmsRun hireco_RAW2DIGI_L1Reco_RECO.py outputFile=step3_%d.root maxEvents=-1 inputFiles=%s\n' % (jobCounter,line) )
            scriptFile.write('cmsRun runForest_PbPb_DATA_75X_Express.py outputFile=HiForest_%d.root maxEvents=-1 inputFiles=file:step3_%d.root\n' % (jobCounter,jobCounter) )
            scriptFile.write('cmsMkdir %s\n' % outdir)
            scriptFile.write('ls\n')
            scriptFile.write('cmsStage step3_%d.root %s/step3_%d.root\n' % (jobCounter,outdir,jobCounter) )
