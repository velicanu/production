...
for line in f:
    if not line.find('root://') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cd -\n')
            scriptFile.write('cp %s/runForest_PbPb_DATA_75X_AOD.py %s \n' % (workBase,jobsBase))
            scriptFile.write('cp %s/runForest_PbPb_DATA_75X_AOD.py .\n' % workBase)
            # scriptFile.write('cp $CMSSW_BASE/src/HeavyIonsAnalysis/JetAnalysis/test/dbFiles/*.db .\n')
            scriptFile.write('cmsRun runForest_PbPb_DATA_75X_AOD.py outputFile=HiForest_%d.root maxEvents=-1 inputFiles=%s\n' % (jobCounter,line) )
