...
for line in f:
    if not line.find('root://') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cmsRun RunExpressProcessingCfg.py outputFile=RAWRECO_%d.root maxEvents=100 inputFiles=%s\n' % (jobCounter,line) )
            scriptFile.write('cmsMkdir %s\n' % outdir)
            scriptFile.write('ls\n')
            scriptFile.write('cmsStage -f RAWRECO_%d_numEvent100.root %s/RAWRECO_%d.root\n' % (jobCounter,outdir,jobCounter) )
            scriptFile.write('rm RAWRECO_%d*root\n' % (jobCounter))
            scriptFile.close()
