...
for line in f:
    if not line.find('root://') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cd -\n')
            scriptFile.write('cp %s/skimTriggersEventContent.py %s \n' % (workBase,jobsBase))
            scriptFile.write('cp %s/skimTriggersEventContent.py .\n' % workBase)
            # scriptFile.write('cp $CMSSW_BASE/src/HeavyIonsAnalysis/JetAnalysis/test/dbFiles/*.db .\n')
            scriptFile.write('cmsRun skimTriggersEventContent.py outputFile=a_%d maxEvents=-1 inputFiles=%s\n' % (jobCounter,line) )
            scriptFile.write('cmsMkdir %s\n' % outdir)
            scriptFile.write('ls\n')
            scriptFile.write('for i in `ls *.root` \n' )
            scriptFile.write('do \n' )
            scriptFile.write('  cmsStage -f $i %s/f%d_$i \n' % (outdir,jobCounter) )
