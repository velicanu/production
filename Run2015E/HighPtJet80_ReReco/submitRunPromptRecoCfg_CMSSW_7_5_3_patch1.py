...
for line in f:
    if not line.find('root://') :
        if not line.startswith('#') :
            ...
            scriptFile.write('cd -\n')
            scriptFile.write('cp %s/RunPromptRecoCfg_CMSSW_7_5_3_patch1.py %s \n' % (workBase,jobsBase))
            scriptFile.write('cp %s/RunPromptRecoCfg_CMSSW_7_5_3_patch1.py .\n' % workBase)
            # scriptFile.write('cp $CMSSW_BASE/src/HeavyIonsAnalysis/JetAnalysis/test/dbFiles/*.db .\n')
            scriptFile.write('cmsRun RunPromptRecoCfg_CMSSW_7_5_3_patch1.py outputFile=RECO_753p1_%d.root maxEvents=201 inputFiles=%s\n' % (jobCounter,line) )
            scriptFile.write('cmsMkdir %s\n' % outdir)
            scriptFile.write('ls\n')
            scriptFile.write('cmsStage -f RECO_753p1_%d_numEvent201.root %s/RECO_753p1_%d_numEvent201.root\n' % (jobCounter,outdir,jobCounter) )
